...
command> ";
    ...
    rm -rf "${tempfolder[@]}";
    rm -rf ./wine-lutris-GE-Proton7-16-x86_64.tar.xz;
    rm -rf ./wine-lutris-GE-Proton7-16-x86_64.tar.xz.1;
    rm -rf ./DivaModManager.zip;
    ...
    sed -i "s#wine#${wine[@]}/lutris-GE-Proton7-16-x86_64/bin/wine64#g" "${modloader[@]}/run_divamodmanager.sh";
    sed -i "s#DivaModManager.exe#${dmm[@]}/DivaModManager.exe#g" "${modloader[@]}/run_divamodmanager.sh";
    clear;
...
command> ";
        ...
        sudo sed -i "s#run_divamodmanager.sh#${modloader[@]}/run_divamodmanager.sh#g" /usr/share/applications/divamodmanager.desktop;
        fi
        if [ $REPLY == "no" ]; then
            clear;
            ./divamodmanager-linux.sh;
